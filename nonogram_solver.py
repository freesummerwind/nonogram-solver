...
class Nonogram(object):
    ...
    def __guess(self):
        """
        Функция, которая проверяет, все ли клетки окрашены в нонограмме, и если не все - то пытается раскрасить
        оставшиеся, угадывая цвет клетки, при этом отсеивая невозможные варианты
    ...
    def solve(self):
        """
        Функция, решающая нонограмму: сначала производится предрасчет для нонограммы, затем вызывается функция,
        реализующая алгоритм решения
